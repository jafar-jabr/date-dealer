...
const DateAndTime = DateDealer.prototype;

DateAndTime.getDatesArray = (startDate, numberOfDates, include = true) => {
...
DateAndTime.getDatesBefore = (startDate, numberOfDates, include) => {
  const dateFormatted = new Date(startDate);
  const maxDays = numberOfDates;
  const datesArray = [];
  let startValue = 1;
...
DateAndTime.getDatesAfter = (startDate, numberOfDates, include) => {
  const dateFormatted = new Date(startDate);
  const maxDays = numberOfDates;
  const datesArray = [];
  let startValue = 1;
...
DateAndTime.addMinutes = (timeString, addMinutes) => {
...
DateAndTime.getDatesBetween = (start, end) => {
  const arr = [];
  const dt = start;
  let theDate = null;
  while (dt <= end) {
    theDate = DateAndTime.formatTheTime(new Date(dt), 'date');
    arr.push(theDate);
...
DateAndTime.getDayIndex = theDate => {
...
DateAndTime.formatTheTime = (UnixTimestamp, type, separator = '/') => {
...
DateAndTime.isWeekEnd = theDate => {
  const dayIndex = DateAndTime.getDayIndex(theDate);
  return dayIndex === 5 || dayIndex === 6;
};

export default DateDealer.prototype;
