...
exports.getDatesBetween = (start, end) => {
  ...
  let theDate = null;
  while (dt <= end) {
    theDate = exports.doFormat(parser.parseToDate(dt), 'yyyy-mm-dd');
    arr.push(theDate);
 ...
 */
exports.dateOf = (date) => {
  return parser.parseToDate(date);
};
 ...
 * @param {string | Date } targetTime
 * @returns {{days: number, hours: number, minutes: number, seconds: number}}
 */
exports.timeSince = (targetTime) => {
  const now = new Date();
  return exports.timeBetween(targetTime, now);
}
 ...
 * @returns {{days: number, hours: number, minutes: number, seconds: number}}
 */
exports.timeUntil = (targetTime) => {
  const now = new Date();
  return exports.timeBetween(now, targetTime);
}
